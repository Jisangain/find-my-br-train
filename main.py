...
def load_data():
    try:
        with open('data.json', 'r') as f:
            data = json.load(f)
            # Add notice fields if they don't exist
            if "persistence_notice" not in data:
                data["persistence_notice"] = {}
            if "first_open_notice" not in data:
                data["first_open_notice"] = ""
            if "app_update_notice" not in data:
                data["app_update_notice"] = ""
            if "data_update_notice" not in data:
                data["data_update_notice"] = ""
...
def find_common_stations(train1_stations, train2_stations):
    """Find common stations between two trains with their indices and times"""
    common = []
    for i, station1 in enumerate(train1_stations):
        ...
        # Only consider stopping stations (type 1 or -1)
...
def find_best_interchange_station(common_stations, from_index_train1, to_index_train2):
    ...
    for station in common_stations:
        # Station must be after 'from' station in train1 and before 'to' station in train2
...
def precalculate_two_train_routes():
    ...
    try:
        with open('two_train_routes.json', 'r') as f:
            ...
            if cached_data.get("revision") == CURRENT_REVISION:
                ...
                for key, value in cached_data.get("routes", {}).items():
                    # Use special separator to handle station names with underscores
        ...
        pass  # File doesn't exist or is invalid, proceed with calculation
    
    print("Starting precalculation of two-train routes...")
    
    tid_to_stations = DATA.get("tid_to_stations", {})
    
    routes = {}  # Format: {(from_sid, to_sid): [(train1_id, train2_id, interchange_station)]}
            ...
            for from_idx, from_station in enumerate(train1_stations):
                ...
                for to_idx, to_station in enumerate(train2_stations):
                    ...
                    if best_interchange and max_wait >= 0:
                        ...
                        # Store only essential data: train1_id, train2_id, interchange_station
    ...
    try:
        with open('two_train_routes.json', 'w') as f:
            # Convert tuple keys and values to JSON-serializable format
            json_routes = {
                "revision": CURRENT_REVISION,  # Save revision number
                "routes": {
                    f"{k[0]}|||{k[1]}": [  # Use ||| separator to handle station names with underscores
...
# Precalculate two-train routes at startup
print("\n" + "="*60)
print("INITIALIZING TWO-TRAIN ROUTE PRECALCULATION")
print("="*60)
TWO_TRAIN_ROUTES = precalculate_two_train_routes()
print("="*60 + "\n")
...
async def get_two_train_routes(from_station: str, to_station: str):
...
async def get_all_two_train_routes():
